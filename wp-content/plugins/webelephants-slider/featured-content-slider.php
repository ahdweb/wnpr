<?php
/*
Plugin Name: Web Elephants Slider
Plugin URI: 
Description: This Plugin is used to show your featured Posts/Pages with thumbnails in a nice slider.
Version: 
Author: Pankaj Bhakhar
Author URI: 
*/

$c_slider_options_page = get_option('siteurl') . '/wp-admin/admin.php?page=webelephants-slider/options.php';

function c_slider_options_page() {
        
	add_options_page('Web Elephants Slider Options', 'Slider', 10, 'webelephants-slider/options.php');
...
function c_slider_add_scripts() {
        ...
        if ( !is_admin() ) {
                ...
                wp_register_script('jquery.cycle', get_bloginfo('url') . '/wp-content/plugins/webelephants-slider/scripts/jquery.cycle.all.2.72.js', array('jquery'), '1.3' );
...
// Code START for Slide custom post type
...
function c_slider_init() {
        ...
        add_meta_box("feat_slider", "Slider Options", "c_slider_meta", "slider", "normal", "high");
...
?>
		...
		<table class="form-table">
			<tr>
				<th style="width: 17%;"><label for="feat_slider">Read More Link URL</label></th>
				<td><input type="text" name="feat_slider" style="width: 100%;" value="<?php echo $feat_slider; ?>" /> </td>
			</tr>
		</table>
	</div>
...
function c_slider_insert($atts, $content = null) {
        ...
        include (ABSPATH . '/wp-content/plugins/webelephants-slider/content-slider.php');
